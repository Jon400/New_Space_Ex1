...
def get_hough_circles(img):
    ...
    circles = cv2.HoughCircles(img.astype(np.uint8), cv2.HOUGH_GRADIENT, 1, 20,
...
def get_stars_data(img, as_pandas=False):
    ...
    if circles is not None:
        ...
        for x, y, r in circles[0]:
            coords.append((x, y, r, img[x, y]))
    coords_arr = np.array(coords) if len(coords) > 0 else coords  # empty np.array raises an error in pd.DataFrame
    if as_pandas:
